...
**Interfaces:**
- Consumes: `analysis/runner.Run` (Task 1), `analysis/internal/storage.New`, `analysis/internal/llm.NewAnthropicClient`.
...
- [ ] **Step 1: Write `mcp/internal/tools/analysis.go`**
...
import (
	...
	"analysis/internal/llm"
	analysisstorage "analysis/internal/storage"
	"analysis/runner"
)
...
// RunAnalysis runs the analysis pipeline via analysis/runner.Run.
func RunAnalysis(ctx context.Context, store *analysisstorage.Store, riskStore *riskstorage.Store, args RunAnalysisArgs) (RunAnalysisResult, error) {
	...
	runID, successCount, err := runner.Run(ctx, store, riskStore, llm.NewAnthropicClient(), args.Assets, args.AssetNames, timeframe, agents)
	if err != nil {
...
- [ ] **Step 2: Write `mcp/internal/tools/analysis_test.go`**
...
func TestRunAnalysis_RequiresAssets(t *testing.T) {
	if _, err := RunAnalysis(context.Background(), nil, nil, RunAnalysisArgs{}); err == nil {
		t.Fatal("expected an error for missing assets, got nil")
	}
}

func TestRunAnalysis_RejectsUnknownAgent(t *testing.T) {
	if _, err := RunAnalysis(context.Background(), nil, nil, RunAnalysisArgs{Assets: []string{"BTC"}, Agents: []string{"not-a-real-agent"}}); err == nil {
		t.Fatal("expected an error for an unknown agent, got nil")
	}
}
```

Both tests exercise only the validation that runs before any store is
touched, so `nil` stores are safe here — deliberately, same pattern as
...
failing with a nil-pointer panic instead of the expected validation
error, that panic is a real bug (validation stopped running before the
store access), not a fixture problem.

- [ ] **Step 3: Register the tool in `mcp/cmd/mcp-server/main.go`**

Add `analysisstorage "analysis/internal/storage"` to the imports, connect
an `analysisStore` alongside the existing `store`/`riskStore` (reusing the
same `dsn`), and register:

```go
	analysisStore, err := analysisstorage.New(ctx, dsn)
	if err != nil {
		return fmt.Errorf("connect analysis storage: %w", err)
	}
...
```go
	mcp.AddTool(server, &mcp.Tool{
		...
		result, err := tools.RunAnalysis(ctx, analysisStore, riskStore, args)
		return nil, result, err
	})
```

(Note: `mcp/internal/tools/analysis.go`'s import of
`analysisstorage "analysis/internal/storage"` and `main.go`'s import of
the same package under the same alias refer to the same type — the tool
function's second parameter and the connection `main.go` passes it must
both be `*analysisstorage.Store` / `*analysis/internal/storage.Store`.)

- [ ] **Step 4: Resolve the new `analysis` import and run the tests**
...
```bash
git add mcp/internal/tools/analysis.go mcp/internal/tools/analysis_test.go mcp/cmd/mcp-server/main.go mcp/go.mod mcp/go.sum
...
**Interfaces:**
- Consumes: `strategist/runner.Run` (Task 2), `strategist/internal/storage.New`, `strategist/internal/storage.DecisionsForTest`-equivalent read-back (see below — `runner.Run` itself doesn't return the decisions, it persists them; this tool reads them back), `strategist/internal/llm.NewAnthropicClient`.
...
query, no `ForTest` framing, since this is now a real production caller.
...
- [ ] **Step 2: Write `mcp/internal/tools/strategist.go`**
...
import (
	...
	"strategist/internal/llm"
	strategiststorage "strategist/internal/storage"
	"strategist/runner"
)
...
// RunStrategist runs the strategist pipeline via strategist/runner.Run,
// then reads back the decisions it persisted (Run itself only returns
// error — see this task's note on DecisionsForRun).
func RunStrategist(ctx context.Context, store *strategiststorage.Store, riskStore *riskstorage.Store, args RunStrategistArgs) (RunStrategistResult, error) {
	...
	if err := runner.Run(ctx, store, riskStore, llm.NewAnthropicClient(), args.AnalysisRunID, args.Assets, timeframe, args.Cash, args.Positions, args.DailyLoss, args.WeeklyLoss, args.Drawdown, args.ConsecutiveLosses); err != nil {
		return RunStrategistResult{}, err
	}

	decisions, err := store.DecisionsForRun(ctx, args.AnalysisRunID)
	if err != nil {
		return RunStrategistResult{}, fmt.Errorf("read back decisions: %w", err)
	}
...
- [ ] **Step 3: Write `mcp/internal/tools/strategist_test.go`**
...
func TestRunStrategist_RequiresAnalysisRunID(t *testing.T) {
	if _, err := RunStrategist(context.Background(), nil, nil, RunStrategistArgs{Assets: []string{"BTC"}, Cash: 1000}); err == nil {
		t.Fatal("expected an error for a missing analysis_run_id, got nil")
	}
}

func TestRunStrategist_RequiresAssets(t *testing.T) {
	if _, err := RunStrategist(context.Background(), nil, nil, RunStrategistArgs{AnalysisRunID: "some-run-id", Cash: 1000}); err == nil {
		t.Fatal("expected an error for missing assets, got nil")
	}
}
```

Same nil-store-is-safe reasoning as Task 6's tests — both validation
checks run before any store is touched.

- [ ] **Step 4: Register the tool in `mcp/cmd/mcp-server/main.go`**

```go
	strategistStore, err := strategiststorage.New(ctx, dsn)
	if err != nil {
		return fmt.Errorf("connect strategist storage: %w", err)
	}
	defer strategistStore.Close()
```
...
```go
	mcp.AddTool(server, &mcp.Tool{
		...
		result, err := tools.RunStrategist(ctx, strategistStore, riskStore, args)
		return nil, result, err
	})
```

- [ ] **Step 5: Resolve the new `strategist` import and run the tests**
...
Also re-run `strategist`'s own test suite directly, since Step 1 modified
...
```bash
cd ../strategist && git add internal/storage/decisions.go && git commit -m "feat(strategist): add DecisionsForRun for production callers"
...
**Interfaces:**
- Consumes: `simulation/runner.RunBacktest`/`runner.Config` (Task 3), `simulation/internal/storage.New`.
- Produces: `tools.RunBacktest` (handler function, tested directly) — registered as the `run_backtest` tool.

- [ ] **Step 1: Write `mcp/internal/tools/backtest.go`**
...
// simulation/runner.RunBacktest, applying the same defaults cmd/backtest's
// CLI flags do for anything left unset (InitialCash, FeePct,
// MAShortPeriod, MALongPeriod) — RunBacktest itself has no opinion on
// defaults, it only validates whatever values it's given.
func RunBacktest(ctx context.Context, riskStore *riskstorage.Store, simStore *simstorage.Store, args RunBacktestArgs) (RunBacktestResult, error) {
	...
	runID, tradeAttempts, results, err := simrunner.RunBacktest(ctx, riskStore, simStore, cfg)
	if err != nil {
...
- [ ] **Step 2: Write `mcp/internal/tools/backtest_test.go`**
...
func TestRunBacktest_RejectsInvertedPeriod(t *testing.T) {
	...
	if _, err := RunBacktest(context.Background(), nil, nil, args); err == nil {
		t.Fatal("expected an error for period_end before period_start, got nil")
	}
}

func TestRunBacktest_AppliesDefaults(t *testing.T) {
	// A missing/invalid driving_timeframe is rejected before any store is
	// touched, regardless of the defaults — this test only confirms the
	// zero-value defaulting doesn't itself cause a spurious validation
	// failure by checking a case that fails for an unrelated reason
	// (driving_timeframe not in timeframes) rather than a defaulting bug.
	start := time.Now()
	args := RunBacktestArgs{
		PeriodStart: start, PeriodEnd: start.Add(time.Hour),
		Timeframes: []string{"1h"}, DrivingTimeframe: "4h", Assets: []string{"BTC"},
	}
	_, err := RunBacktest(context.Background(), nil, nil, args)
	if err == nil {
...
```go
	mcp.AddTool(server, &mcp.Tool{
		...
		result, err := tools.RunBacktest(ctx, riskStore, simStore, args)
		return nil, result, err
	})
```

- [ ] **Step 4: Resolve the new `simulation` import and run the tests**
...
```bash
git add mcp/internal/tools/backtest.go mcp/internal/tools/backtest_test.go mcp/cmd/mcp-server/main.go mcp/go.mod mcp/go.sum
...
```go
func newServer(store *storage.Store, riskStore *riskstorage.Store, analysisStore *analysisstorage.Store, strategistStore *strategiststorage.Store, simStore *simstorage.Store) *mcp.Server {
	server := mcp.NewServer(&mcp.Implementation{Name: "investment-platform", Version: "0.1.0"}, nil)
...
func run(ctx context.Context) error {
	...
	server := newServer(store, riskStore, analysisStore, strategistStore, simStore)
	return server.Run(ctx, &mcp.StdioTransport{})
}
```
...
deliberately named identically to the local variables Tasks 4-8 connected
(`store`, `riskStore`, `analysisStore`, `strategistStore`, `simStore`), so
every closure captures the same names it already did; the only thing that
changes is that those names now come from `newServer`'s parameter list
instead of `run`'s local `:=` declarations.
...
func testStores(t *testing.T) (*storage.Store, *riskstorage.Store, *analysisstorage.Store, *strategiststorage.Store, *simstorage.Store) {
	t.Helper()
	dsn := os.Getenv("TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("TEST_DATABASE_URL not set, skipping integration tests")
	...
	analysisStore, err := analysisstorage.New(ctx, dsn)
	if err != nil {
		t.Fatalf("analysisstorage.New: %v", err)
	}
	t.Cleanup(analysisStore.Close)
	strategistStore, err := strategiststorage.New(ctx, dsn)
	if err != nil {
		t.Fatalf("strategiststorage.New: %v", err)
	}
	t.Cleanup(strategistStore.Close)
	simStore, err := simstorage.New(ctx, dsn)
	if err != nil {
		t.Fatalf("simstorage.New: %v", err)
	}
	t.Cleanup(simStore.Close)
	return store, riskStore, analysisStore, strategistStore, simStore
}
...
func TestServer_ListsAllSixTools(t *testing.T) {
	store, riskStore, analysisStore, strategistStore, simStore := testStores(t)
	server := newServer(store, riskStore, analysisStore, strategistStore, simStore)
