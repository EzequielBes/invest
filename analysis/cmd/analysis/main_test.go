...
func TestRun_AllAgentsSucceed(t *testing.T) {
	...
	runID, successCount, err := Run(ctx, store, riskStore, client, []string{"NOSUCHASSET"}, "1h", []string{"technical", "risk_context"})
...
func TestRun_PartialLLMFailureStillCompletes(t *testing.T) {
	...
	runID, successCount, err := Run(ctx, store, riskStore, client, []string{"NOSUCHASSET"}, "1h", []string{"risk_context", "news"})
	t.Cleanup(func() { store.DeleteRunForTest(ctx, runID) })
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
